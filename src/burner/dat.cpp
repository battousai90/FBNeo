...
// Bootcade fork: write what the driver knows and a DAT never said.
//
// A DAT describes ROMs. Genre, series, player count and hiscore support are
// declared by every driver in its BurnDriver struct, and were reachable only
// by parsing the source, which meant re-implementing the C preprocessor to
// tell an active flag from one commented out. Writing them here, from the
// running binary, removes that whole class of mistakes: the emulator states
// what it supports, once, in the file that is already published.
//
// Extra elements inside <game>. Parsers that do not know them skip them,
// the way they already skip <comment>.

struct DatFlagName { INT32 nFlag; const char* szName; };

// Labels are FBNeo's own, from src/burner/win32/string.rc, repeated here
// because those are Win32 resources and the DAT is generated by the SDL
// build.
static const DatFlagName DatGenreNames[] = {
	...
	{ GBF_CASINO,     "Casino" },
	{ GBF_BALLPADDLE, "Ball and Paddle" },
	{ GBF_MAZE,       "Maze" },
...
static const DatFlagName DatFamilyNames[] = {
...
// Several flags can be set at once: a game is "Platformer, Action". The
// comma separates values; no label contains one, so a reader can split on
// it without ambiguity. "/" cannot be used for that: it already appears
// inside single labels.
static void DatWriteFlagList(FILE* fDat, const char* szTag, INT32 nFlags, const DatFlagName* pNames)
{
	char szList[512];
	szList[0] = 0;

	for (INT32 i = 0; pNames[i].szName; i++) {
		if ((nFlags & pNames[i].nFlag) == 0) continue;
		if (szList[0]) strncat(szList, ", ", sizeof(szList) - strlen(szList) - 1);
		strncat(szList, pNames[i].szName, sizeof(szList) - strlen(szList) - 1);
	}
	if (!szList[0]) return;

	char szEscaped[512];
	memset(szEscaped, 0, sizeof(szEscaped));
	ReplaceAmpersand(szEscaped, szList);
	fprintf(fDat, "\t\t<%s>%s</%s>\n", szTag, szEscaped, szTag);
}

static void DatWriteDriverInfo(FILE* fDat)
{
	DatWriteFlagList(fDat, "genre", BurnDrvGetGenreFlags(), DatGenreNames);
	DatWriteFlagList(fDat, "family", BurnDrvGetFamilyFlags(), DatFamilyNames);

	INT32 nPlayers = BurnDrvGetMaxPlayers();
	if (nPlayers > 0) fprintf(fDat, "\t\t<players>%d</players>\n", nPlayers);

	// Hiscore support as the DRIVER declares it, which is a different
	// question from whether an online service ranks the game.
	if (BurnDrvGetFlags() & BDF_HISCORE_SUPPORTED) fprintf(fDat, "\t\t<hiscore>1</hiscore>\n");
}
